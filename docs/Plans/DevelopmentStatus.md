...
`dev-fedora`，Qt 6.11.1，GCC 16.1.1：编译零警告，**21 个测试目标全过**。
...
- 首次启动单页提示、Linux AppImage 应用菜单入口（第 12.2、12.4 节）。
- **Windows 侧实测**：托盘恢复、隐藏后从托盘唤回、二次启动唤回，都要等综合校验。本批只在 Linux 上实测。
- Linux 工作区的**平台实现**。设置模型与界面项已就位，但 `DeskPetWindowBackend` 还没有对应方法，`workspacePinning` 在两个后端里都是假。需要 XCB 侧实现（`_NET_WM_DESKTOP`），Windows 按第 3.4 节维持不支持。
