...
**该检查点已于 2026-08-27 通过**：参数冻结为 `docs/Decisions.md` 第 4.8 节，`1.5×` 正式采用，正式气泡随后实现。
...
- 可实时调节全部取值、切换 `1×`／`1.5×`／`2×` 与任意一段台词，并把当前取值导出为可粘贴的清单。
...
- `src/dialogue/LineSelection` 按触发场景分组挑选单页台词。分组内容取自第 4.4／4.5 节的「使用场景」一列。
...
| `tst_bubblelayout` | 第 4.8 节全部取值回查决策文档、宽度不随文字长短变化、不低于最小高度、偏移公式、**角色处于屏幕四角 × `1×`／`1.5×`／`2×`** 全部落在可用区域内、靠左缘时夹取而非镜像、**36 个页面在三档倍率下都不溢出面板** |
...
- 双平台 CI 结果待确认。
- **Windows 侧实际渲染未验证**：`1.5×` 的非均匀像素、气泡置顶层次、气泡窗口不进任务栏，都要等综合校验。
- 气泡与角色的显示倍率目前只能取整数：`CharacterWindow` 的接口是 `int integerScale`，`--scale` 也只收整数。第 4.8 节的布局本身已经支持 `1.5×` 并有测试覆盖，但要让产品真的跑在 `1.5×`，需要把角色侧的倍率放宽到小数。这属于计划第 12 节（设置）的「显示倍率」条目，计划中该条仍写作「离散**整数**显示倍率」，与已经采用 `1.5×` 的决策不一致，需要项目所有者确认后统一。
- 四条两页闲聊文案（`authored-02`、`08`、`10`、`15`）暂未进入随机气泡池。两页台词按第 4.1 节属于连续对话，需要点击翻页、不会自动消失；是否允许自主闲聊选中它们，需要项目所有者决定并写回决策文档。
- 与行为钩子相关的触发场景（拖动、接近鼠标、进入休息、开始／结束行走、投喂各阶段、掉落的替代反应）对应的台词已在文案库中，但触发它们的行为事件尚未实现，因此暂未分组。
- `probes/bubble/` 与正式实现各有一份渲染代码。原型只用于视觉审核，不参与产品构建路径之外的正确性保证；正式实现改动时不会自动同步原型。
