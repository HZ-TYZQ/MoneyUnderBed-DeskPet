...
`dev-fedora`，Qt 6.11.1，GCC 16.1.1：编译零警告，**25 个测试目标全过**。
...
| `tst_singleinstance` | 端点名带应用 ID 与用户区分后缀且同用户下稳定、首个实例成为主实例、第二个实例退让并唤回第一个、遗留端点不阻塞启动、主实例退出后端点可重新拿下 |
...
- **`SingleInstance` 的唤回在 Windows 上会静默丢失**，这是产品代码的真 bug，不是测试写错。客户端原先写完就断开，Unix socket 上数据留在缓冲区里服务端照样读得到，Windows 命名管道上服务端可能读到空数据，于是消息对不上、唤回被丢弃。改为一次应答握手：服务端读满期望长度、回一个字节再断开，客户端收到应答后才关闭。保留的行为是「连上了就退让」—— 握手慢不是「没有实例在跑」的证据。
...
计划第 12.3 节写的是「Windows 使用命名 mutex 保证唯一实例，再用本地 IPC 发送唤回」。实现改为**两平台统一使用 Qt 本地套接字**（Linux 是 Unix socket，Windows 是命名管道）。

理由：第 8.4 节要求优先使用 Qt，能力不足时才下沉到平台层，而这里 Qt 已经足够 —— 端点名被占用本身就是「已有实例」的判据，互斥量提供的是同一个信息。计划担心的启动竞态由「连不上又监听失败就再连一次」处理，不需要额外的互斥量。

代价：判定依赖端点而非内核对象，因此遗留端点必须显式清理，这一点已实现并有测试覆盖。若项目所有者认为仍应按原文加互斥量，改动只限于 `SingleInstance::acquire()`。
