...
`dev-fedora`，Qt 6.11.1，GCC 16.1.1：编译零警告，**20 个测试目标全过**。
...
- 托盘（第 12.1 节）。
- **隐藏**。第 3.3 节要求隐藏后必须有唤回通道，因此等托盘或单实例唤回就位后再加，不提供藏了就找不回来的入口。
- 单实例与唤回（第 12.3 节）。
- 首次启动单页提示、Linux AppImage 应用菜单入口、应用与托盘图标（第 12.2、12.4 节）。
...
- **一个已知的语义缺口**：第 12.1 节要求「连续对话期间打开或关闭右键菜单不结束、不翻页，也不重置当前页面」。菜单路径本身完全不触碰对话会话，因此不会翻页也不会重置。但 `QMenu::exec()` 期间事件循环继续运行，对话的打字和第 4.1 节的 20 秒无操作超时都照常计时；菜单挂着超过 20 秒，对话仍会按超时规则结束。要严格满足该条，需要在菜单打开期间冻结对话，而这不能只停定时器 —— 产品用的是单调时钟，恢复时经过的时间会一次性补上，必须同时平移 `DialogueSession` 的时间基准。是否值得为此改动已通过测试的状态机，留给项目所有者决定。
