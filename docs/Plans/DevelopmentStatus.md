...
- Linux 工作区一项按**后端能力自述**决定是否显示，不是判断当前运行在哪个系统上。当前两个后端的 `workspacePinning` 都是假，因此该项在两个平台上都隐藏。
...
| `tst_settingswindow` | 平台不支持时工作区项隐藏且不参与设置、支持时显示、刷新控件不反过来上报、改动即时上报完整设置、恢复默认单独上报、诊断信息含第 5.3 节全部字段、诊断信息不含家目录／`PATH=`／用户名 |
...
| `tst_backendfactory` | 任何平台都给出可用后端、后端名带平台前缀、置顶／像素命中／穿透／不进窗口列表四项能力两平台都自述支持、拿不到 X11 连接时不自称支持工作区固定、空窗口不崩 |
...
### 第三批：工作区、首次启动提示与应用菜单入口

- `src/platform/LinuxWindowBackend` 是 Linux 侧第一个平台专用后端。它**只覆盖工作区归属一项** —— Qt 没有跨平台接口表达它，必须走 EWMH 的 `_NET_WM_DESKTOP`；其余能力全部继承 `QtWindowBackend`，因为 Linux 所需的其他窗口能力早已由纯 Qt XCB 路径实测通过。
- `workspacePinning` 只在真的拿得到 X11 连接时才自述为真。否则设置界面会显示一个点了没反应的选项。
- 属性和客户端消息两条路都走：属性用于窗口尚未映射的情况，客户端消息用于已映射的窗口 —— EWMH 规定已映射窗口要由窗口管理器代改，只写属性不一定生效。
...
一度实现为两平台统一用 Qt 本地套接字兼当唯一性锁，2026-08-27 由项目所有者纠正，改回计划第 12.3 节的原方案：
...
- **真实桌面实测**。以下都只有代码路径，没有真实桌面结果：工作区固定与切换（需要 KDE Plasma 宿主）、托盘恢复、隐藏后从托盘唤回、AppImage 的应用菜单入口与二次启动唤回、Windows 托盘。容器内只验证了「无托盘时退回再次启动唤回」和单实例的端到端行为。
- 完整整数倍率集合待确认。当前实现集合是 `1×`／`2×`／`3×`，决策文档第 13 节仍把它列为未定，`3×` 是否正式列入设置界面需要项目所有者确认。
- 阶段 7 退出门的三条都还没有真实桌面结果，因此状态维持 `进行中`。
